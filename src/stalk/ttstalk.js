...
module.exports = function (app) {
  app.get('/stalk/tiktok', async (req, res) => {
    ...
    if (!user)
      return res.json({ status: false, error: 'User is required' });

    try {
      const anu = await tiktokStalk(user);
      if (anu.status === 'error') return res.json(anu);

      // Tambahkan statistik dari stats ke user
      ...
      anu.user.videoCount = anu.stats.videoCount; // <--- Jumlah postingan
