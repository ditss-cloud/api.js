...
module.exports = function (app) {
  app.get('/primbon/ramalancinta', async (req, res) => {
    try {
      const {
      ...
      const result = await primbon.ramalan_cinta(
        nama1, tgl1, bln1, thn1,
        nama2, tgl2, bln2, thn2
      );

      if (!result.status) {
        return res.status(500).json({ status: false, error: result.message || 'Gagal meramal cinta' });
      }

      res.json({
        status: true,
        jenis: 'Primbon Ramalan Cinta',
