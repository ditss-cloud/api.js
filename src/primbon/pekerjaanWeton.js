...
module.exports = function (app) {
  app.get('/primbon/pekerjaanweton', async (req, res) => {
    try {
      ...
      if (!result.status) {
        return res.status(500).json({ status: false, error: result.message || 'Gagal mencari pekerjaan weton' });
      }

      res.json({
        status: true,
        jenis: 'Primbon Pekerjaan Weton',
