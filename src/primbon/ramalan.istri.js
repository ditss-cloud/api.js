...
module.exports = function (app) {
  app.get('/primbon/suamiistri', async (req, res) => {
    try {
      const {
      ...
      const result = await primbon.suami_istri(
        nama1, tgl1, bln1, thn1,
        nama2, tgl2, bln2, thn2
      );

      if (!result.status) {
        return res.status(500).json({ status: false, error: result.message || 'Gagal meramal suami istri' });
      }

      res.json({
        status: true,
        jenis: 'Primbon Ramalan Suami Istri',
