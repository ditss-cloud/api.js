...
module.exports = function (app) {
  app.get('/primbon/sifatusahabisnis', async (req, res) => {
    try {
      ...
      if (!result.status) {
        return res.status(500).json({ status: false, error: result.message || 'Gagal mencari sifat usaha bisnis' });
      }

      res.json({
        status: true,
        jenis: 'Primbon Sifat Usaha Bisnis',
